...
export default function AppPicker({ onSelect, onCancel }: AppPickerProps) {
  ...
  useEffect(() => {
    ScreenlyEnforcer.getInstalledApps()
      .then((list: AppInfo[]) => {
        const sorted = list
      ...
      .catch(() => setNativeUnavailable(true))
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity
                  ...
                  onPress={() => onSelect({ name: item.appName, packageName: item.packageName })}
                >
